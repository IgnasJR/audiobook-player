...
const ViewAlbum = ({ token, setError }) => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchAlbum = async () => {
      try {
        ...
        setError({
          type: "error",
          message: "Failed to fetch album. Please try again later.",
        });
        setAlbum(null);
      }
    };
  ...
  useEffect(() => {
    if (album) {
      originalProgress.current = {
        progress: album.trackProgress || 0,
        track: album.track || null,
      };
    }
  }, [album]);
      ...
      {album ? (
        <div className="flex flex-wrap justify-center bg-inherit md:pt-16 pt-5 md:pb-16 pb-28 z-0 text-white">
          <div className="flex flex-col items-center">
            <img
              src={album.coverArtLink}
              alt="Album Cover"
              className="md:w-[30vw] w-[80vw] rounded-xl mb-6"
            />
            <h1 className="text-4xl">{album.albumName}</h1>
            <h1 className="text-2xl">{album.artist}</h1>
            <button
              onClick={() => setIsAlbumSelected(true)}
              className="px-16 py-4 hover:bg-amber-600 bg-slate-700 rounded-xl mt-4 "
            >
              {album.track === 0 && album.trackProgress === 0
                ? "Start "
                : "Continue"}
            </button>
          </div>
          <ul className="md:w-1/4 w-3/4 md:ml-5 overflow-y-auto md:h-[80vh]">
            {album.tracks.map((track, index) => (
              <li
                className="rounded-md border-black border bg-slate-700 hover:transition-colors hover:bg-amber-600 m-1"
                key={track.id}
